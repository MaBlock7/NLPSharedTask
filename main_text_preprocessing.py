...
def preprocess_garbage_data(df_null: pd.DataFrame) -> pd.DataFrame:
    # Add is_abstract dummy to zofa
    df_null['is_abstract'] = 1
    # Rename columns
    df_null = df_null.rename(columns={'abstract': 'text',
                                      'sdg_label': 'label'})
    # Drop if no abstract
    df_null = df_null.dropna(subset='text')
    # Clean OSDG data
    df_null['language'] = df_null.text.apply(detect_language)
    # Filter non-english texts out
    df_null = df_null[df_null.language == 'en'].copy()

    return df_null[['text', 'label', 'is_abstract']]
...
def save_data(df: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    file_path = f"{args.output_dir}/cleaned_data{null_included}{synth_included}.csv"
...
def main(args):
    ...
    if args.include_null:
        ...
        df_null_prep = preprocess_garbage_data(df_null)
